...
EOF
  ...
  cat >> /tmp/bootstrap.sql <<EOF
CREATE USER '$MAXSCALE_USER'@'%' IDENTIFIED '$MAXSCALE_USER_PASSWORD';
GRANT SUPER, RELOAD, REPLICATION CLIENT, REPLICATION SLAVE, SHOW DATABASES, FILE ON *.* TO '$MAXSCALE_USER'@'%';
